...
export function JobItem(props: InferProps<typeof JobItem.propTypes>): ReactElement {
  ...
  const CellSpacer = (): ReactElement => <TableSpacer color={theme.colors.sections.experience.spacerBackground} />;
  ...
  if (experienceScope.length > 1) {
    ...
    for (let idx = 1; idx <= experienceScope.length - 1; idx++) {
      el.push(<span key={(idx - 1).toString()}>{experienceScope[idx - 1]}</span>);
